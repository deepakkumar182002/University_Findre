...
const map = L.map('map').setView([24.5, 82], 6); // Centered around India
        ...
        // Define the boundaries for different regions
        const uttarPradeshBounds = [[24.5, 80], [30.5, 84]];
        const madhyaPradeshBounds = [[21.5, 74], [26.5, 82]];
        const delhiBounds = [[28.5, 76.5], [28.9, 77.3]];
        const biharBounds = [[24, 83], [27, 88]];
        const karnatakaBounds = [[12, 74], [18, 78]];
        const gujaratBounds = [[20, 68], [24.5, 75]];
        const newDelhiBounds = [[28.5, 76.5], [28.9, 77.3]];
        const haryanaBounds = [[28, 75], [30, 77]];

        // Event listener for map click
        map.on('click', function (e) {
            // Check if the click is within the boundaries of different regions
            if (isWithinBounds(e.latlng, uttarPradeshBounds)) {
                window.open('uttar_pradesh.html', '_blank');
            } else if (isWithinBounds(e.latlng, madhyaPradeshBounds)) {
                window.open('madhya_pradesh.html', '_blank');
                ...
                window.open('bihar.html', '_blank');
            } else if (isWithinBounds(e.latlng, karnatakaBounds)) {
                window.open('karnataka.html', '_blank');
            } else if (isWithinBounds(e.latlng, gujaratBounds)) {
                window.open('gujarat.html', '_blank');
            } else if (isWithinBounds(e.latlng, newDelhiBounds)) {
                window.open('new_delhi.html', '_blank');
            } else if (isWithinBounds(e.latlng, haryanaBounds)) {
                window.open('haryana.html', '_blank');
            }
        });
